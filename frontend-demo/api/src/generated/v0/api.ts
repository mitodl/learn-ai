 ...
 *
 * @export
 * @interface ProblemAndSolutionResponse
 */
export interface ProblemAndSolutionResponse {
  /**
   *
   * @type {string}
   * @memberof ProblemAndSolutionResponse
   */
  problem: string
  /**
   *
...
/**
 * TutorProblemApi - axios parameter creator
 * @export
 */
export const TutorProblemApiAxiosParamCreator = function (
  configuration?: Configuration,
) {
  return {
    /**
     * Return a sample tutor problem and solution.
     * @param {string} problem_code problem_code of the problem
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
     */
    tutorProblemRetrieve: async (
      problem_code: string,
      options: RawAxiosRequestConfig = {},
    ): Promise<RequestArgs> => {
      // verify required parameter 'problem_code' is not null or undefined
      assertParamExists("tutorProblemRetrieve", "problem_code", problem_code)
      const localVarPath = `/api/v0/tutor_problem/`.replace(
        `{${"problem_code"}}`,
        encodeURIComponent(String(problem_code)),
      )
      // use dummy base URL string because the URL constructor only accepts absolute URLs.
      const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL)
      let baseOptions
...
/**
 * TutorProblemApi - functional programming interface
 * @export
 */
export const TutorProblemApiFp = function (configuration?: Configuration) {
  const localVarAxiosParamCreator =
    TutorProblemApiAxiosParamCreator(configuration)
  return {
    /**
     * Return a sample tutor problem and solution.
     * @param {string} problem_code problem_code of the problem
    ...
    async tutorProblemRetrieve(
      ...
      (
        axios?: AxiosInstance,
        basePath?: string,
      ) => AxiosPromise<ProblemAndSolutionResponse>
    > {
      const localVarAxiosArgs =
        await localVarAxiosParamCreator.tutorProblemRetrieve(
          problem_code,
          options,
        )
      const index = configuration?.serverIndex ?? 0
      const operationBasePath =
        operationServerMap["TutorProblemApi.tutorProblemRetrieve"]?.[index]?.url
      return (axios, basePath) =>
        createRequestFunction(
          localVarAxiosArgs,
          globalAxios,
...
/**
 * TutorProblemApi - factory interface
 * @export
 */
export const TutorProblemApiFactory = function (
  configuration?: Configuration,
  basePath?: string,
  axios?: AxiosInstance,
) {
  const localVarFp = TutorProblemApiFp(configuration)
  return {
    /**
     * Return a sample tutor problem and solution.
     * @param {TutorProblemApiTutorProblemRetrieveRequest} requestParameters Request parameters.
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
     */
    tutorProblemRetrieve(
      requestParameters: TutorProblemApiTutorProblemRetrieveRequest,
      options?: RawAxiosRequestConfig,
    ): AxiosPromise<ProblemAndSolutionResponse> {
      return localVarFp
        .tutorProblemRetrieve(requestParameters.problem_code, options)
        .then((request) => request(axios, basePath))
    },
  }
}
...
/**
 * Request parameters for tutorProblemRetrieve operation in TutorProblemApi.
 * @export
 * @interface TutorProblemApiTutorProblemRetrieveRequest
 */
export interface TutorProblemApiTutorProblemRetrieveRequest {
  /**
   * problem_code of the problem
   * @type {string}
   * @memberof TutorProblemApiTutorProblemRetrieve
   */
  readonly problem_code: string
}

/**
 * TutorProblemApi - object-oriented interface
 * @export
 * @class TutorProblemApi
 * @extends {BaseAPI}
 */
export class TutorProblemApi extends BaseAPI {
  /**
   * Return a sample tutor problem and solution.
   * @param {TutorProblemApiTutorProblemRetrieveRequest} requestParameters Request parameters.
   * @param {*} [options] Override http request option.
   * @throws {RequiredError}
   * @memberof TutorProblemApi
   */
  public tutorProblemRetrieve(
    requestParameters: TutorProblemApiTutorProblemRetrieveRequest,
    options?: RawAxiosRequestConfig,
  ) {
    return TutorProblemApiFp(this.configuration)
      .tutorProblemRetrieve(requestParameters.problem_code, options)
      .then((request) => request(this.axios, this.basePath))
  }
}
